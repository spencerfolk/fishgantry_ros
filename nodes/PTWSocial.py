...
class PersistentFishSocial():
    ...
    """
    def __init__(self,sigma_u=0.3034,theta_u=14.6096,mu_u=0.0757, sigma_zdot=0.0873,mu_zdot=0.0114,theta_zdot=1,sigma_w=2.85,theta_w=2.74,mu_w=-0.02,sigma_o=0.1145,fc=0,dfish = 0.1,dist_K = 10000000.0,yaw_K = 10000000.0):
...
#             fc=sigma_w
    ...
    def updateStates(self,dt,Omega,U,x,y,zdot,psi,S,state=1,friendpos=None):
        if friendpos is not None:
            ...
            angle_error_2 = goal_angle+(self.laps-1)*2*math.pi-self.psi
            ...
            # goal_angle=0
            dist_error = -self.dfish + sqrt((self.x-friendpos[0])**2+(self.y-friendpos[1])**2)
        ...
        self.dt = dt
        Omegadot, Udot, zdoubledot, dw = self.calcDerivatives(Omega,U,x,y,zdot,psi,friendpos)
...
############################################################################
    ...
    def drivePersistentFish(self,dt,state=1,friendpos=None):
        ...
        self.updateStates(dt,self.Omega,self.U,self.x,self.y,self.zdot,self.psi,self.S,state,friendpos)
...
class MarkovChainSocial(PersistentFishSocial):
    # Inherits a persistentfish and runs a 2-state Markov chain with time-dependent transition matrices.
    def __init__(self,sigma_u=0.3034,theta_u=14.6096,mu_u=0.0757, sigma_zdot=0.0873,mu_zdot=0.0114,theta_zdot=1,sigma_w=2.85,theta_w=2.74,mu_w=-0.02,sigma_o=0.1145,fc=0,dfish = 0.1,dist_K = 10000,yaw_K = 10000):
        # self.tau_active = 4.5846
        # self.tau_inactive = 1.7966
        PersistentFishSocial.__init__(self,sigma_u,theta_u,mu_u, sigma_zdot,mu_zdot,theta_zdot,sigma_w,theta_w,mu_w,sigma_o,fc,dfish ,dist_K ,yaw_K )  # Initialize markov as a child of PersistentFish
