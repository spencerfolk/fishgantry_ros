...
class Window():
    ...
    def init_window(self):
        ...
        self.sU = Scale(self.master,from_=0,to=200,orient=HORIZONTAL,length=200)
    ...
    def loop(self):
        ...
        if len(line)>0:
            ...
            if len(splitline)==6:
                self.ardt,f1,f2,f3,f4,f5 = float(splitline[0]),float(splitline[1]),float(splitline[2]),float(splitline[3]),float(splitline[4]),float(splitline[5])
